...
describe('authSession handler', () => {
  ...
  it('creates session for valid admin ID token', async () => {
    const { CognitoJwtVerifier } = require('aws-jwt-verify');
    const { createSessionForUser } = require('../../src/db/dynamodb/sessionsRepository');
    const { createSessionToken } = require('../../src/utils/sessionToken');

    // Mock manager and contractor verifiers to fail, admin to succeed
    CognitoJwtVerifier.create
      .mockReturnValueOnce({
        verify: jest.fn().mockRejectedValue(new Error('Invalid token')),
      })
      ...
      .mockReturnValueOnce({
        verify: jest.fn().mockResolvedValue({ sub: 'user789', email: 'admin@example.com' }),
      });
    ...
    const event = { body: JSON.stringify({ idToken: 'admin-token' }) } as APIGatewayProxyEventV2;
    const res = await handler(event);
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body!);
    expect(body).toEqual({
