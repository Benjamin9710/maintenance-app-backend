import { handler } from '../../src/handlers/adminListManagers';
import { requireAdminSession } from '../../src/utils/sessionAuth';
import { listManagers } from '../../src/services/cognitoManagers';
...
describe('adminListManagers', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('returns list of managers for admin session', async () => {
    const mockSession = {
      sub: 'admin123',
    ...
    (listManagers as jest.Mock).mockResolvedValue(mockResult);
    ...
    expect(listManagers).toHaveBeenCalledWith('', { limit: undefined, paginationToken: undefined });
  });
  ...
  it('returns 500 on unexpected error', async () => {
    ...
    expect(body.message).toBe('Unable to list managers');
  });
});
